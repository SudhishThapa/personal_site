...
# Limit total request size (e.g., 25 MB)
...
def ensure_column(conn, table, column):
    """Add a column if missing (SQLite)."""
    cur = conn.cursor
    cur = conn.cursor()
...
def init_db():
    ...
    # Ensure new column for multi-images
...
def _fix_rel_path(p: str) -> str:
    """
    Make sure stored image paths are relative to UPLOAD_FOLDER:
...
def admin_new():
    ...
    if request.method == 'POST':
        ...
        cur.execute('UPDATE posts SET image_path=?, image_paths=? WHERE id=?',
